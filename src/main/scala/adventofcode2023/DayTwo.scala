...
object DayTwo extends App {
  ...
  def stringsToGames(input: List[String]): List[Game] =
    input.map { case s"Game $id: $setsStr" =>
      val sets = setsStr.split("; ").toList.map { setStr =>
        setStr.split(", ").foldLeft(RedGreenBlue(0, 0, 0)) { case (rgb, str) =>
          str match {
            case s"$n red"   => rgb.copy(r = n.toInt)
            case s"$n green" => rgb.copy(g = n.toInt)
            case s"$n blue"  => rgb.copy(b = n.toInt)
            case _           => rgb
          }
        }
      }
  ...
  println(part1(input, rule)) // 2486
  println(part2(input)) // 87984
}
